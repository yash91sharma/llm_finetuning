...
Handles the complete training, testing, and validation pipeline for GPT-2 fine-tuning using LoRA.
"""
...
def test_model(model_path, config):
    ...
    tokenizer_path = os.path.join(config['model']['save_path'], f"{base_model_name}-tokenizer")
...
def main():
    ...
    parser.add_argument(
        "--test-only",
        type=str,
        help="Path to LoRA adapter directory for testing only"
    )
