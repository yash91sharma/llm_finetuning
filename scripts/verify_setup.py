...
def check_transformers():
    ...
    if version.parse(transformers.__version__) >= version.parse(min_version):
        ...
        print(f"⚠️  Consider upgrading transformers to {min_version} or higher")
...
def check_memory():
    ...
    try:
        ...
        if total_gb >= 16:
            ...
            print("⚠️  Limited memory - consider smaller models or batch sizes")
        else:
            print("❌ Insufficient memory for LLM training")
            
        return total_gb >= 8
    except ImportError:
        print("\n⚠️  psutil not installed - cannot check memory")
        return True
