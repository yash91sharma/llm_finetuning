...
def load_and_setup_model(config, device):
    ...
    if not os.path.exists(model_path):
        logging.error(
            f"Model not found at {model_path}. Please run download_model.py first."
        )
        return None, None

    if not os.path.exists(tokenizer_path):
        logging.error(
            f"Tokenizer not found at {tokenizer_path}. Please run download_model.py first."
        )
        return None, None
...
def main():
    ...
    parser.add_argument(
        "--test-only", action="store_true", help="Only test device compatibility"
    )
